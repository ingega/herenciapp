...
from src.api.v1.apps.expenses.models import Expenses, ExpenseCategory
from src.api.v1.apps.expenses.schemas import ExpenseCreate, ExpenseUpdate


def make_expense(date: datetime, expense: str = "Item", category: ExpenseCategory = ExpenseCategory.OPERATION, quantity: float = 1.0, total: float = 10.0) -> Expenses:
...
def test_expense_service_crud(session: Session):
    ...
    # add expense
    payload = ExpenseCreate(expense="Coffee", total=2.5, category=ExpenseCategory.BEVERAGES)
