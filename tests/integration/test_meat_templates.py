...
# ==============================================================================
# TEST 1: Unauthenticated User (No Cookie / Missing Credentials) for list endpoint
# ==============================================================================
def test_meat_page_unauthenticated_redirects(client):
    """
    ...
    response = client.get("/orders/flavors/meat/list", follow_redirects=False)
...
# ==============================================================================
def test_meat_page_authenticated_success(client, authorized_client_cookies):
    """
    ...
    # Assert 1: Verify it stayed on the meat catalogue page (No redirection triggered!)
    ...
    assert "Agregar Nueva Opción de carne" in response.text
    assert "Selección de carne" in response.text
...
# ==============================================================================
# TEST 3: Unauthenticated User (No Cookie / Missing Credentials) for create tempalate endpoint
# ==============================================================================
def test_meat_create_page_unauthenticated_redirects(client):
    """
    Ensure that accessing the protected UI route without an active session
    or auth cookie results in a clean redirect or unauthorized handling.
    """
    # Act: Explicitly prevent following redirects to capture the raw 303 response
    response = client.get("/orders/flavors/meat/add/ui", follow_redirects=False)
...
# ==============================================================================
# TEST 2: Authenticated User (Cookie present + HTML assertions) in create template endpoint
# ==============================================================================
def test_meat_create_page_authenticated_success(client, authorized_client_cookies):
    """
    Ensure that an authorized request loads the products workspace template layout,
    injects session structures safely, and skips auth fallback paths.
    """
    ...
    # Assert 3: Text content visual validation
    assert "Asignar opción de carne" in response.text
    assert "(lengua, buche, cuerito, etc)" in response.text
